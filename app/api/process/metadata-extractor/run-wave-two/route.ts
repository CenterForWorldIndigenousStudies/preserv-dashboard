...
type TriggerConfig = { pipelineBaseUrl: string; triggerToken: string; callbackToken: string }
...
function requireTriggerConfig(): TriggerConfig {
...
function validateBatchForWaveTwo(batchId: string, batch: Awaited<ReturnType<typeof getProcessBatchStatus>>): NextResponse | null {
  ...
  if (!['completed', 'completed_with_failures'].includes(batch.metadataExtractor?.openaiBatchWave1?.status ?? '')) {
    return NextResponse.json(
      { error: `Wave one must be completed before wave two can start for batch ${batchId}.` },
      { status: 400 },
    )
  }
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  const validationError = validateBatchForWaveTwo(batchId, batch)
  if (validationError) {
  ...
  try {
    const response = await fetch(new URL('/metadata-extractor/run-wave-two', triggerConfig.pipelineBaseUrl), {
      method: 'POST',
      headers: {
      ...
      const errorMessage =
        typeof payload.error === 'string'
          ...
          : typeof payload.detail === 'string'
            ? payload.detail
            : 'Failed to queue metadata extractor wave two.'
      throw new Error(errorMessage)
    }

    logEvent('info', 'metadata_extractor_wave_two_requested', {
      batchId,
      requestId,
      startedBy,
      route: METADATA_EXTRACTOR_RUN_WAVE_TWO_PATH,
    })
    return NextResponse.json(payload, { status: response.status })
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : 'Failed to queue metadata extractor wave two.'
    logEvent('error', 'metadata_extractor_wave_two_failed', {
      batchId,
      requestId,
      startedBy,
