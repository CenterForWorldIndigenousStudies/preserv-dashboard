...
const detail = {
  ...
  metadata: [
    ...
    {
      name: 'cost_saved',
      value: JSON.stringify({ value: 0.123456 }),
      value_type: 'number',
      notes: 'Estimated cost saved by reprocessing this batch.',
    },
  ],
}
...
describe('BatchDetailPage', () => {
  ...
  it('renders the batch identity, processing properties, and return link', async () => {
    ...
    mockGetPipelineExecutionSnapshot.mockResolvedValue({
      batch: { batchId: 'batch-1', pipelineRequestedStages: ['ocr_processor'] },
    ...
    expect(markup).not.toContain('Started At')
    ...
    expect(markup).toContain('batch-purpose')
