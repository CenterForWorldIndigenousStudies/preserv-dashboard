...
export function parseMetadataValue(
  ...
  if (effectiveType === "url" || effectiveType === "link") {
    ...
    return {
      display: <a href={url} target="_blank" rel="noopener noreferrer" className="text-[#355834] hover:underline">{url}</a>,
      plainText: url,
    };
  }
...
function formatLinkableText(text: string): ParsedMetadataValue {
  ...
  if (/^https?:\/\//i.test(trimmed)) {
    return {
      display: <a href={trimmed} target="_blank" rel="noopener noreferrer" className="text-[#355834] hover:underline">{trimmed}</a>,
      plainText: trimmed,
    };
  }
...
function formatJsonDisplay(jsonString: string): ParsedMetadataValue {
  return {
    display: (
      <pre style={{ fontSize: "0.75rem", backgroundColor: "#f4f1f0", padding: "0.75rem", borderRadius: "0.5rem", overflowX: "auto", whiteSpace: "pre-wrap", fontFamily: "monospace" }}>
        {jsonString}
      </pre>
    ),
